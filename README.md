...
![원점수 비교](./resource/score_ratio_final.png)
...
최종 정답을 풀이의 마지막에 하는 것 보다 풀이의 처음에 말하게 하는 것이 점수가 더욱 높게 나왔습니다.

→ GPT-4 내부에서 Decoding 전략 중 Beam shearch와 같은 방식을 택하여, 최종 정답에 등장할 가능성이 매우 높은 단어들인 ‘1번’, ‘2번’, ‘3번’, ‘4번’, ‘5번’으로 시작하는 경우의 수(풀이)를 검토함으로써 좋은 성능을 낸 것으로 보입니다.

선택지마다의 판단에 대해 지문 속 근거가 된 문장을 뽑도록 하는 것이 점수가 더욱 높게 나왔습니다.

→ 수능 국어는 지문에서 모든 근거를 찾을 수 있도록 만들었기 때문에 풀이를 (선택지 5번에 대한 답변) + "(지문 속 근거가 된 문장)"으로 설정해준 것이 매우 유효한 프롬프트 였던 것으로 보입니다. GPT-4의 높은 한국어 기계 독해 능력을 보여주는 것으로 보입니다.
